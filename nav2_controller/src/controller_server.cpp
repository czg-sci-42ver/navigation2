...
ControllerServer::on_configure(const rclcpp_lifecycle::State& state)
{
  ...
  RCLCPP_INFO(
    get_logger(), "Controller frequency set to %.4fHz", controller_frequency_);
  ...
  try {
    progress_checker_type_ =
      nav2_util::get_plugin_type_param(node, progress_checker_id_);
    progress_checker_ =
      progress_checker_loader_.createUniqueInstance(progress_checker_type_);
    RCLCPP_INFO(get_logger(),
                "Created progress_checker : %s of type %s",
                progress_checker_id_.c_str(),
                progress_checker_type_.c_str());
    progress_checker_->initialize(node, progress_checker_id_);
  } catch (const pluginlib::PluginlibException& ex) {
    RCLCPP_FATAL(get_logger(),
                 "Failed to create progress_checker. Exception: %s",
                 ex.what());
  ...
  for (size_t i = 0; i != goal_checker_ids_.size(); i++) {
    try {
      ...
      RCLCPP_FATAL(get_logger(),
                   "Failed to create goal checker. Exception: %s",
                   ex.what());
      return nav2_util::CallbackReturn::FAILURE;
    }
  }
  ...
  RCLCPP_INFO(get_logger(),
              "Controller Server has %s goal checkers available.",
  ...
  for (size_t i = 0; i != controller_ids_.size(); i++) {
    try {
      ...
      controller->configure(
        node, controller_ids_[i], costmap_ros_->getTfBuffer(), costmap_ros_);
      controllers_.insert({ controller_ids_[i], controller });
    } catch (const pluginlib::PluginlibException& ex) {
      RCLCPP_FATAL(
        get_logger(), "Failed to create controller. Exception: %s", ex.what());
      return nav2_util::CallbackReturn::FAILURE;
    }
  }
  ...
  speed_limit_sub_ = create_subscription<nav2_msgs::msg::SpeedLimit>(
    ...
    rclcpp::QoS(10),
    std::bind(
      &ControllerServer::speedLimitCallback, this, std::placeholders::_1));
...
bool
ControllerServer::findControllerId(const std::string& c_name,
...
bool
ControllerServer::findGoalCheckerId(const std::string& c_name,
...
void
ControllerServer::setPlannerPath(const nav_msgs::msg::Path& path)
{
  RCLCPP_DEBUG(get_logger(),
               "Providing path to the controller %s",
  ...
  nav_2d_utils::transformPose(costmap_ros_->getTfBuffer(),
...
void
ControllerServer::computeAndPublishVelocity()
{
  ...
  if (!progress_checker_->check(pose)) {
    ...
    RCLCPP_INFO(this->get_logger(),
                "2Test progress_checker_->check result %d\n",
  ...
  RCLCPP_DEBUG(
    get_logger(), "Publishing velocity at time %.2f", now().seconds());
  publishVelocity(cmd_vel_2d);
}

void
ControllerServer::updateGlobalPath()
{
  ...
  const geometry_msgs::msg::TwistStamped& velocity)
{
  ...
  if (vel_publisher_->is_activated() &&
      vel_publisher_->get_subscription_count() > 0) {
    vel_publisher_->publish(std::move(cmd_vel));
  }
}

void
ControllerServer::publishZeroVelocity()
{
...
ControllerServer::isGoalReached()
{
  ...
  return goal_checkers_[current_goal_checker_]->isGoalReached(
    pose.pose, end_pose_, velocity);
}

bool
ControllerServer::getRobotPose(geometry_msgs::msg::PoseStamped& pose)
{
...
void
ControllerServer::speedLimitCallback(
...
ControllerServer::dynamicParametersCallback(
