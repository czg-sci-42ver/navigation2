...
ControllerServer::ControllerServer(const rclcpp::NodeOptions &options)
    : nav2_util::LifecycleNode("controller_server", "", false, options),
      ...
      default_ids_{"FollowPath"}, default_types_{"dwb_core::DWBLocalPlanner"} {
...
ControllerServer::on_configure(const rclcpp_lifecycle::State &state) {
  ...
  RCLCPP_INFO(get_logger(), "Controller Server has %s goal checkers available.",
  ...
  speed_limit_sub_ = create_subscription<nav2_msgs::msg::SpeedLimit>(
      ...
      std::bind(&ControllerServer::speedLimitCallback, this,
                std::placeholders::_1));
...
void ControllerServer::setPlannerPath(const nav_msgs::msg::Path &path) {
  ...
  nav_2d_utils::transformPose(costmap_ros_->getTfBuffer(),
                              costmap_ros_->getGlobalFrameID(), end_pose,
                              end_pose, tolerance);
...
void ControllerServer::computeAndPublishVelocity() {
  ...
  try {
    cmd_vel_2d = controllers_[current_controller_]->computeVelocityCommands(
        pose, nav_2d_utils::twist2Dto3D(twist),
